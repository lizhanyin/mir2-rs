...
use mlibrary_v1::MLibraryV1;
use std::path::{Path, PathBuf};

/// 从资源路径加载库并获取指定范围的图像预览
///
/// # 参数
/// - `resource_path`: 资源根目录
...
///
/// # 返回
/// 返回 (LibraryInfo, Vec<Option<RgbaImage>>)
pub fn load_images_from_path(
    resource_path: &Path,
    library_name: &str,
    start: usize,
    end: usize,
) -> Result<(LibraryInfo, Vec<Option<image::RgbaImage>>)> {
    // 构建库文件路径
    let extensions = [".wzl", ".wil", ".Lib"];
    ...
    let lib_path = lib_path
        .ok_or_else(|| LibraryError::ParseError(format!("找不到库文件: {}", library_name)))?;

    // 加载库
    let (info, mut loader) = LibraryLoader::load(&lib_path)?;

    // 获取图像预览
    let previews = loader.get_previews_range(start, end)?;

    Ok((info, previews))
}

/// 从资源路径加载库并获取指定索引列表的图像预览
///
/// # 参数
/// - `resource_path`: 资源根目录
/// - `library_name`: 库文件名（不含扩展名）
...
/// # 返回
/// 返回 (LibraryInfo, Vec<Option<RgbaImage>>)
pub fn load_images_by_indices_from_path(
    resource_path: &Path,
    library_name: &str,
    indices: &[usize],
) -> Result<(LibraryInfo, Vec<Option<image::RgbaImage>>)> {
    ...
    // 获取图像预览
    let previews = loader.get_previews_by_indices(indices)?;
...
impl LibraryLoader {
    ...
    /// 替换图像
    pub fn replace_image(&mut self, index: usize, image: &mlibrary_v2::MImage) -> Result<()> {
    ...
    /// 添加图像
    pub fn add_image(&mut self, image: &mlibrary_v2::MImage) -> Result<()> {
        tracing::debug!("添加新图像");
    ...
    ///
    /// # 返回
    /// 返回一个 Vec，其中每个元素是对应索引的图像预览（可能为 None）
    pub fn get_previews_range(
        &mut self,
        start: usize,
        end: usize,
    ) -> Result<Vec<Option<image::RgbaImage>>> {
        let count = self.image_count();
        let end = end.min(count);
        let start = start.min(end);

        tracing::debug!("批量获取图像预览: start={}, end={}", start, end);

        let mut previews = Vec::with_capacity(end - start);
        for index in start..end {
            let preview = self.get_preview(index)?;
    ...
    /// # 返回
    /// 返回一个 Vec，其中每个元素是对应索引的图像预览（可能为 None）
    pub fn get_previews_by_indices(
        &mut self,
        indices: &[usize],
    ) -> Result<Vec<Option<image::RgbaImage>>> {
        tracing::debug!("批量获取图像预览: indices={:?}", indices);
    ...
    ///
    /// # 返回
    /// 返回一个 Vec，其中每个元素是对应索引的图像信息
    pub fn get_image_infos_range(&mut self, start: usize, end: usize) -> Result<Vec<ImageInfo>> {
        let count = self.image_count();
        let end = end.min(count);
        let start = start.min(end);

        tracing::debug!("批量获取图像信息: start={}, end={}", start, end);

        let mut infos = Vec::with_capacity(end - start);
        for index in start..end {
    ...
    /// # 返回
    /// 返回一个 Vec，其中每个元素是对应索引的图像信息
    pub fn get_image_infos_by_indices(&mut self, indices: &[usize]) -> Result<Vec<ImageInfo>> {
        tracing::debug!("批量获取图像信息: indices={:?}", indices);
